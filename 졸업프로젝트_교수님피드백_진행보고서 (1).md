...
1. 연구의 중심 기여를 **정확도 향상**보다 **최근성·intent를 이용한 long-tail/diversity trade-off 제어**로 잡는 것이 적절한지 궁금합니다.
2. 현재 intent 추출과 metadata KG가 동시에 들어가는데, 논문에서 LLM intent의 기여를 더 명확히 보이려면 어떤 ablation 구성이 적절할지 의견을 듣고 싶습니다.
3. warm temporal 환경과 genuine cold-start 환경에서 결과가 달랐습니다. cold-start를 본 연구의 핵심 주장으로 유지해야 하는지, 아니면 제외하고 후속 과제로 두는 것이 좋은지 궁금합니다.
4. LLM 기반 intent 추출 비용을 통합 호출·offline batch로 줄였으나, 실제 서비스 latency까지는 검증하지 않았습니다. 학부 졸업프로젝트 수준에서 효율성 평가는 어느 범위까지 제시하면 충분한지 궁금합니다.
5. intent specificity를 entropy/IDF로 정의한 것이 충분히 설득력 있는지, 또는 더 단순하고 명확한 weighting 기준이 좋은지 의견을 받고 싶습니다.
6. Amazon Clothing 외 MovieLens/Goodreads 등 다른 도메인에서의 재현 실험을 어느 수준까지 해야 일반화 가능성을 논의할 수 있을지 궁금합니다.
